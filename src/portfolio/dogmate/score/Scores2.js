...
let changesImported = {};
const changedCellRenderer = function (
  instance,
  td,
  row,
  col,
...
export default class Scores2 extends React.Component {
  constructor(props) {
    super(props);
    this.urlPrefix =
      (window.location.host === "localhost:7777" ? "http://localhost" : "") +
    ...
    this.state = {
      ...
      options: {
        ...
        columns: [
          { data: "sitter", readOnly: true, className: "htLeft menu-callable" },
          ...
          {
            data: "rate_repurchase",
            type: "numeric",
            format: "0%",
            readOnly: true,
          },
          {
            data: "count_advance_meetings",
            type: "numeric",
            readOnly: true,
            validator: digitRegexp,
            allowInvalid: false,
          },
          {
            data: "count_edu_attendance",
            type: "numeric",
            readOnly: true,
            validator: digitRegexp,
            allowInvalid: false,
          ...
          {
            data: "rate_care_journal_write",
            type: "numeric",
            format: "0%",
            readOnly: true,
          },
          {
            data: "count_personal_pr",
            type: "autocomplete",
            readOnly: true,
            source: ["", 0, 1],
            strict: true,
            ...
            switch (key) {
              ...
              case "import-sitters":
                setTimeout(() => {
                  ...
                  axios
                    .post(this.urlPrefix + "/statistics", { sitters: sitters })
                    .then((response) => {
                    ...
                    .catch((response) => {
                      axios
                        .get("/mockups/scores-statistics.json")
                        .then((response2) => {
  ...
  componentDidMount() {
    ...
    if (toolbar) {
      this.setState({ ...this.state, toolbarHeight: toolbar.clientHeight });
    }

    axios
      .get(window.location.pathname + "/check-auth")
      .then((response) => {
        // console.log(response);
        this.setState(
          update(this.state, { authority: { $set: response.data.authority } })
        );
      })
      .catch((response) => {
        let mock = { authority: true };
        this.setState(
          update(this.state, { authority: { $set: mock.authority } })
        );
      });
  }
  ...
  fetchPeriods_proc(data) {
    let p = this.state.period
      ? this.state.period
      : data[data.length > 1 ? 1 : 0].period_no;
      ...
      .then((response) => {
        ...
        if (isInitial) {
          ...
          for (let idx in entries) {
            idx = parseInt(idx, 10);
            entries[idx].entry_weight = Number.parseFloat(
              entries[idx].entry_weight
            );
          ...
          this.setState(
            update(this.state, { entries: { $set: response.data.entries } })
          );
          this.table.updateSettings({ colHeaders: cols });
        }
      })
      .catch((response) => {
        axios.get("/mockups/dogmate/scores-initial.json").then((response2) => {
          ...
          if (isInitial) {
            ...
            this.setState(
              update(this.state, { entries: { $set: response2.data.entries } })
            );
            this.table.updateSettings({ colHeaders: cols });
          }
        });
  ...
  refresh(period) {
    axios
      .get(this.urlPrefix + "/fetch", { params: { period: period } })
      .then((response) => {
        // console.log(response);
      ...
      .catch((response) => {
        axios.get("/mockups/dogmate/scores-fetch-1.json").then((response2) => {
  ...
  calc(row, idxRow) {
    ...
    if (row.count_customers !== null) {
      val =
        row.count_customers > 0
          ? ((row.count_reviewers / row.count_customers) * 100).toFixed(0)
          : 0;
      ...
      total += Number.parseFloat(
        (val * entries[seq++].entry_weight).toFixed(1)
      );
    }
    ...
    if (schedules.length) {
      let avgSchedule =
        schedules.reduce((prev, curr) => prev + curr) / schedules.length;
    ...
    if (!isNaN(countAdvanceMeetings) && row.count_customers !== null) {
      val =
        row.count_customers > 0
          ? ((countAdvanceMeetings / row.count_customers) * 100).toFixed(0)
          : 0;
    ...
    if (!isNaN(countEduAttendance) && !isNaN(countEdu)) {
      val =
        countEdu > 0 ? ((countEduAttendance / countEdu) * 100).toFixed(0) : 0;
    ...
    if (!isNaN(countCareJournals) && row.count_deals !== null) {
      rateCareJournalWrite =
        row.count_deals > 0
          ? Number.parseFloat((countCareJournals / row.count_deals).toFixed(2))
          : 0;
    ...
    this.apply_change(
      row,
      { rate_care_journal_write: rateCareJournalWrite },
      "rate_care_journal_write",
      changes,
      idxRow
    );
    ...
    if (!isNaN(countAbandonments) && row.count_deals !== null) {
      val =
        row.count_deals > 0
          ? ((countAbandonments / row.count_deals) * 100).toFixed(0)
          : 0;
    ...
    let revisionaryScore = Number.parseFloat(
      Number.parseFloat(row.revisionary_score).toFixed(1)
    );
          ...
          <Paper style={{ height: "100%" }}>
            <Toolbar ref="toolbar">
              <ToolbarGroup firstChild={true}>
                ...
                {"최근 변경: " +
                  (this.state.updated ? this.state.updated : "저장한 적 없음")}
              </ToolbarGroup>
              <ToolbarGroup lastChild={true}>
                <RaisedButton
                  label="내부데이터 계수 실행"
                  style={{
                    display: this.state.authority ? "inline-block" : "none",
                  }}
                  disabled={!this.state.canImport}
                  onClick={() => {
                    axios
                      .get(this.urlPrefix + "/statistics")
                      .then((response) => {
                        // console.log(response);
                      ...
                      .catch((response) => {
                        axios
                          .get("/mockups/dogmate/scores-statistics.json")
                          .then((response2) => {
                ...
                <RaisedButton
                  ...
                  onClick={() => {
                    if (this.validateForPublish()) {
                      axios
                        .post(this.urlPrefix + "/finish-period", {
                        ...
                        .catch((response) => {
                          ...
                          this.show_snackbar(
                            "마감하면서 뭔가 오류가 생겼어요. 담당 개발자에게 문의해주세요-"
                          );
                        });
                    } else {
                      this.show_snackbar(
                        "마감하려면 모든 데이터를 입력하여 점수가 산출돼야 합니다. 확인해주세요~"
                      );
                    }
                  }}
                />
